...
WIN32_Operation_Proactor::WIN32_Operation_Proactor( bool result_delete  )
...
WIN32_Operation_Proactor::WIN32_Operation_Proactor( u_long milli_seconds , bool result_delete )
: m_completion_port_( NULL )
, result_delete_( result_delete )
{
	open( milli_seconds );
...
void WIN32_Operation_Proactor::close (void)
{
	...
	for (;;)
	{
		...
		WIN32OperationOverlapped *asynch_result =
			(WIN32OperationOverlapped *) overlapped;
...
int WIN32_Operation_Proactor::handle_events (unsigned long milli_seconds)
{
	...
	{
		WIN32OperationOverlapped *asynch_result = (WIN32OperationOverlapped *) overlapped;
		u_long error = 0;
		if( !result )
			error = GetLastError();
...
void WIN32_Operation_Proactor::application_specific_code (WIN32OperationOverlapped *asynch_result,
...
int WIN32_Operation_Proactor::post_completion (WIN32OperationOverlapped *result )
{
	if( is_null( result ) )
		ThrowException1( IllegalArgumentException, "result" );
		

	WIN32OperationOverlapped* res = result;
	if( is_null( res ) )
		ThrowException( CastException );

	DWORD bytes_transferred = 0;
	Hazel_HANDLE handle = this->get_handle ();  
