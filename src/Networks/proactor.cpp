...
void proactor::close (void)
{
	...
	for (;;)
	{
		...
		jomoo_io_request *asynch_result =
			(jomoo_io_request *) overlapped;
...
int proactor::handle_events (unsigned long milli_seconds)
{
	...
	{
		jomoo_io_request *asynch_result = (jomoo_io_request *) overlapped;
		u_long error = 0;
		if( !result )
			error = GetLastError();
...
void proactor::application_specific_code (jomoo_io_request *asynch_result,
...
bool proactor::post_completion (jomoo_io_request *result )
{
	if( is_null( result ) )
		return false;
