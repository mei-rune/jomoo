 ...
 * -*- C++ -*-
 * -------------------------------------------------------------------------------
 * - ¤╭⌒╮ ╭⌒╮		                 message_block.h,v 1.0 2004/12/13 09:48:34
 *  ╱◥██◣ ╭╭ ⌒╮
 * ︱田︱田田| ╰------
 * -------------------------------------------------------------------------------
 */
...
public:
	 ...
	 */
	data_block( char* msg_data, size_t len , Message_Flags flag = DONT_DELETE ,BT_Memory_Base* alloc_ = 0 );
	...
	*/
	...
	BT_Memory_Base *get_Memory_Manager( );
...
public:
	 ...
	 * 会试图调用默认的allocator来删除同msg_data.
	 */
	message_block( char* msg_data, size_t len , Message_Flags flag = DONT_DELETE , BT_Memory_Base* alloc_ = 0 );
	 ...
	 * 会试图调用默认的allocator来删除同msg_data.
	 */
	message_block( data_block* data , Message_Flags flag = DONT_DELETE , BT_Memory_Base* allocator_ = 0 );
