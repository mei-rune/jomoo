...
public:
	...
	static const tchar* ISO8601_FORMAT = _T("%Y-%m-%dT%H:%M:%S%z");
	...
	static const tchar* RFC822_FORMAT = _T("%w, %e %b %y %H:%M:%S %Z");
	...
	static const tchar* RFC1123_FORMAT = _T("%w, %e %b %Y %H:%M:%S %Z");
	...
	static const tchar* HTTP_FORMAT = _T("%w, %d %b %Y %H:%M:%S %Z");
	...
	static const tchar* RFC850_FORMAT = _T("%W, %e-%b-%y %H:%M:%S %Z");
	...
	///   Saturday, 1 Jan 05 12:00:00 +0100
	///   Saturday, 1 Jan 05 11:00:00 GMT
	static const tchar* RFC1036_FORMAT = _T("%W, %e %b %y %H:%M:%S %Z");
	...
	static const tchar* ASCTIME_FORMAT = _T("%w %b %f %H:%M:%S %Y");

	/// A simple, sortable date/time format.
	///
	/// Example:
	///   2005-01-01 12:00:00
	static const tchar* SORTABLE_FORMAT = _T("%Y-%m-%d %H:%M:%S");



	// names used by formatter and parser
	/// English names of week days (Sunday, Monday, Tuesday, ...).
	static const tchar* WEEKDAY_NAMES[] = const tchar*[7]
	{
		_T("Sunday"),
		_T("Monday"),
		_T("Tuesday"),
	...
	static const tchar* MONTH_NAMES[] = const tchar*[12]
	{
		_T("January"),
		_T("February"),
		_T("March"),
	...
	static tstring format(const DateTime& dateTime, const tchar* fmt, int timeZoneDifferential = UTC);
	{
		...
		while ( it )
		{
			...
			switch (*it)
			{
			case _T('w'): result.append(WEEKDAY_NAMES[dateTime.dayOfWeek()], 0, 3); break;
			case _T('W'): result.append(WEEKDAY_NAMES[dateTime.dayOfWeek()]); break;
			case _T('b'): result.append(MONTH_NAMES[dateTime.month() - 1], 0, 3); break;
			case _T('B'): result.append(MONTH_NAMES[dateTime.month() - 1]); break;
